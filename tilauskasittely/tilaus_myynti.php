    ...
    if (mysql_num_rows($meapu) == 1 and $toimitustapa != '') {
      ...
      if ($meapurow["kateinen"] != "") {
        ...
        if ($meapu2row["nouto"] == "" and $laskurow['tilaustyyppi'] != "W") {
          if($kukarow['oletus_varasto'] and is_numeric($kukarow['oletus_varasto'])) {
            $apuqu = "SELECT toimitustapa.selite 
                      FROM toimitustapa 
                      JOIN varastopaikat on (varastopaikat.yhtio = toimitustapa.yhtio) 
                      WHERE toimitustapa.yhtio = 'mergr' 
                      AND toimitustapa.selite LIKE concat(varastopaikat.nimi,'%') 
                      AND varastopaikat.tunnus = $kukarow[oletus_varasto] 
                      AND toimitustapa.nouto = 'o' 
                      ORDER BY jarjestys 
                      LIMIT 1";
          } else {
            $apuqu = "SELECT *
  ...
  if ((int) $kukarow["kesken"] > 0) {
    ...
    if ($rivilaskuri > 0) {
      ...
      foreach ($rows as $row) {
        ...
        echo "<td $class id='tuotenimitystaul' align='left'>{$liitekuvat}".t_tuotteen_avainsanat($row, "nimitys")."$extranet_tarkistus_teksti</td>";
