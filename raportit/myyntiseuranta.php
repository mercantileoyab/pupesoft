...
if ($lopetus == "") {
  ...
  $monivalintalaatikot = array("ASIAKASOSASTO", "ASIAKASRYHMA", "ASIAKASPIIRI", "<br>DYNAAMINEN_ASIAKAS", "<br>OSASTO", "TRY", "ALERYHMA", "TUOTEMERKKI", "MALLI/MALLITARK", "<br>DYNAAMINEN_TUOTE", "<br>LASKUMYYJA", "TUOTEMYYJA", "ASIAKASMYYJA", "TUOTEOSTAJA", "<br>TOIMIPAIKKA", "KUSTP", "KOHDE", "PROJEKTI");
  ...
  if ($tee == 'go') {
    ...
    foreach ($apu as $i => $mukaan) {
      ...
      if ($mukaan == "aleryhmittain") {
        $group .= ",Aleryhm‰";
        $select .= "if (perusalennus.ryhma!=perusalennus.selite, concat_ws(' - ', perusalennus.ryhma, perusalennus.selite), perusalennus.ryhma) as Aleryhm‰, ";
        $order  .= "Aleryhm‰,";
        $gluku++;

        if ($rajaus[$i] != "") {
          $lisa .= " and Aleryhm‰ LIKE '%{$rajaus[$i]}%' ";
        }
