...
/*
1.    EE100  Sales 24% VAT
1.1    EE110  Sales 20% VAT
1.2    EE120  Sales 22% VAT
2.    EE200  Sales  9% VAT
2.1    EE210  Sales  5% VAT
2.2    EE212  Sales  13% VAT
3.    EE300  Sales  0% VAT
...
4.    EE400  VAT from sales: EE400=(EE100*24%)+(EE100*22%)+(EE100*20%)+(EE100*13%)+(EE100*9%)+(EE100*5%)
...
if ((isset($vv) and $vv > '2025') or (isset($vv) and $vv == '2025' and (isset($kk) and $kk > '6'))) {
  $oletus_verokanta = 24;
}
else {
  $oletus_verokanta = 22;
...
if (isset($tee) and $tee == 'erittele') {
  ...
  if ($ryhma == '1') {
    ...
    $tiliointilisa = " and tiliointi.vero = 24 ";
  }
  elseif ($ryhma == '1.1') {
    $taso = "ee100";
    $eetasolisa = " or alv_taso like '%ee110%'";
  ...
  elseif ($ryhma == '2.1') {
    $taso = "ee100";
    $eetasolisa = " or alv_taso like '%ee110%'";
    $tiliointilisa = " and tiliointi.vero = 5 ";
  }
  elseif ($ryhma == '2.2') {
    $taso = "ee100";
    $eetasolisa = " or alv_taso like '%ee110%'";
    $tiliointilisa = " and tiliointi.vero = 13 ";
  }
  elseif ($ryhma == '3') {
    $taso = "ee300";
...
function alvlaskelma($kk, $vv) {
  ...
  if (isset($kk) and $kk != '') {
    ...
    //4. VAT from sales: ($ee100*24%)+($ee100*22%)+($ee100*20%)+($ee100*9%)+($ee100*5%)+($ee100*13%)
    $ee400 = round(($ee100["24.00"] * 0.24) + ($ee100["22.00"] * 0.22) + ($ee100["20.00"] * 0.20) + ($ee100["9.00"] * 0.09) + ($ee100["5.00"] * 0.05) + ($ee100["13.00"] * 0.13), 2);

    //4.05. Sales with VAT: ($ee100*24%)+($ee100*22%)+($ee100*20%)+($ee100*9%)+($ee100*5%)+($ee100*13%)
    $ee405 = round($ee100["24.00"] + $ee100["22.00"] + $ee100["20.00"] + $ee100["9.00"] + $ee100["5.00"] + $ee100["13.00"], 2);
    ...
    // Verollinen myynti
    echo "<tr class='aktiivi'><td><a href = '?tee=erittele&ryhma=1&vv=$vv&kk=$kk&etsivirheita=$etsivirheita'>1)</a> 24% määraga maksustatavad toimingud ja tehingud</td><td align='right'>".sprintf('%.2f', $ee100["24.00"])."</td></tr>";
    echo "<tr class='aktiivi'><td>&raquo; <a href = '?tee=erittele&ryhma=1.1&vv=$vv&kk=$kk&etsivirheita=$etsivirheita'>1<sup>1</sup>)</a> 20% määraga maksustatavad toimingud ja tehingud</td><td align='right'>".sprintf('%.2f', $ee100["20.00"])."</td></tr>";
    echo "<tr class='aktiivi'><td>&raquo; <a href = '?tee=erittele&ryhma=1.2&vv=$vv&kk=$kk&etsivirheita=$etsivirheita'>1<sup>2</sup>)</a> 22% määraga maksustatavad toimingud ja tehingud</td><td align='right'>".sprintf('%.2f', $ee100["22.00"])."</td></tr>";
    echo "<tr class='aktiivi'><td><a href = '?tee=erittele&ryhma=2&vv=$vv&kk=$kk&etsivirheita=$etsivirheita'>2)</a> 9% määraga maksustatavad toimingud ja tehingud</td><td align='right'>".sprintf('%.2f', $ee100["9.00"])."</td></tr>";
    echo "<tr class='aktiivi'><td>&raquo; <a href = '?tee=erittele&ryhma=2.1&vv=$vv&kk=$kk&etsivirheita=$etsivirheita'>2<sup>1</sup>)</a> 5% määraga maksustatavad toimingud ja tehingud</td><td align='right'>".sprintf('%.2f', $ee110["5.00"])."</td></tr>";
    echo "<tr class='aktiivi'><td>&raquo; <a href = '?tee=erittele&ryhma=2.2&vv=$vv&kk=$kk&etsivirheita=$etsivirheita'>2<sup>2</sup>)</a> 13% määraga maksustatavad toimingud ja tehingud</td><td align='right'>".sprintf('%.2f', $ee110["13.00"])."</td></tr>";

    // Väärät alvikannat
    foreach ($ee100 as $eekey => $eeval) {
      if ($eekey != "24.00" and $eekey != "22.00" and $eekey != "20.00" and $eekey != "9.00" and $eekey != "5.00" and $eekey != "13.00") echo "<tr><td>XXX ".($eekey * 1)."% määraga maksustatavad toimingud ja tehingud</td><td align='right'>".sprintf('%.2f', $eeval)."</td></tr>";
    }
    foreach ($ee110 as $eekey => $eeval) {
      if ($eekey != "24.00" and $eekey != "22.00" and $eekey != "20.00" and $eekey != "9.00" and $eekey != "5.00" and $eekey != "13.00") echo "<tr><td>XXX ".($eekey * 1)."% määraga maksustatav kauba voi teenuse omatarve</td><td align='right'>".sprintf('%.2f', $eeval)."</td></tr>";
    }
    ...
    echo "<tr class='aktiivi'><td>4) Käibemaks kokku (24% lahtrist 1 + 20% lahtrist 1<sup>1</sup> + 22% lahtrist 1<sup>2</sup> + 9% lahtrist 2 + 5% lahtrist 2<sup>1</sup> + 13% lahtrist 2<sup>2</sup>)</td><td align='right'>".sprintf('%.2f', $ee405)."</td></tr>";
    
    echo "<tr class='aktiivi'><td>&raquo; 4<sup>1</sup>) Impordilt tasumisele kuuluv käibemaks (+)</td><td align='right'>".sprintf('%.2f', $ee410)."</td></tr>";
    ...
    echo "<tr class='aktiivi'><td>&raquo; 5.3) Ettevõtluses (100%) kasutatava sõiduauto soetamiselt ja sellise sõiduauto tarbeks kaupade soetamiselt ja teenuste saamiselt tasutud või tasumisele kuuluv käibemaks</td><td align='right'>".sprintf('%.2f', $ee530)."</td></tr>";
    
    echo "<tr class='aktiivi'><td>&raquo; <a href = '?tee=erittele&ryhma=5.4&vv=$vv&kk=$kk&etsivirheita=$etsivirheita'>5.4)</a> Osaliselt ettevõtluses kasutatava sõiduauto soetamiselt ja sellise sõiduauto tarbeks kaupade soetamiselt ja teenuste saamiselt tasutud või tasumisele kuuluv käibemaks</td><td align='right'>".sprintf('%.2f', $ee540)."</td></tr>";
    ...
    echo "<tr class='aktiivi'><td>&raquo; 7.1) Erikorra alusel maksustatava kinnisasja, metallijäätmete, väärismetalli ja metalltoodete soetamine (KMS § 41<sup>1</sup>)</td><td align='right'>".sprintf('%.2f', $ee710)."</td></tr>";

    echo "<tr class='aktiivi'><td>8) Maksuvaba käive</td><td align='right'>".sprintf('%.2f', $ee800)."</td></tr>";

    echo "<tr class='aktiivi'><td>9) Erikorra alusel maksustatava kinnisasja, metallijäätmete, kullamaterjali ja väärismetalli käive (KMS § 41<sup>1</sup>)</td><td align='right'>".sprintf('%.2f', $ee900)."</td></tr>";
