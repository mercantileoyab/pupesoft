...
function alvlaskelma($kk, $vv) {
  ...
  if (isset($kk) and $kk != '') {
    ...
    //4. VAT from sales: ($ee100*20%)+($ee100*9%)
    $ee400 = round(($ee100["22.00"] * 0.22) + ($ee100["20.00"] * 0.20) + ($ee100["9.00"] * 0.09) + ($ee100["5.00"] * 0.05), 2);

    //4.05. Sales with VAT: ($ee100*22%)+($ee100*20%)+($ee100*9%)+($ee100*5%)
    $ee405 = round($ee100["22.00"] + $ee100["20.00"] + $ee100["9.00"] + $ee100["5.00"], 2);
    ...
    foreach ($ee100 as $eekey => $eeval) {
      if ($eekey != "22.00" and $eekey != "20.00" and $eekey != "9.00" and $eekey != "5.00") echo "<tr><td>XXX ".($eekey * 1)."% määraga maksustatavad toimingud ja tehingud</td><td align='right'>".sprintf('%.2f', $eeval)."</td></tr>";
    }
    foreach ($ee110 as $eekey => $eeval) {
      if ($eekey != "22.00" and $eekey != "20.00" and $eekey != "9.00" and $eekey != "5.00") echo "<tr><td>XXX ".($eekey * 1)."% määraga maksustatav kauba voi teenuse omatarve</td><td align='right'>".sprintf('%.2f', $eeval)."</td></tr>";
    }
